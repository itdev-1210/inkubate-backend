...
import {
  INKUBATE_ADDRESS,
  INKUBATE_ADDRESS_BSC,
  INKUBATE_ADDRESS_RYOSHI,
  LAUNCHPAD_ADDRESS,
  LAUNCHPAD_ADDRESS_BSC,
  LAUNCHPAD_ADDRESS_RYOSHI,
} from '@config/address';
...
import { resolveIpfs } from '@common/helper';
...
export class Web3Service {
  ...
  constructor(
    ...
    this.web3 = {
      BNB: new Web3(this.configService.get('urls.BSC_RPC')),
      MAIN: new Web3(this.configService.get('urls.INFURA_URL')),
      RYOSHI: new Web3(this.configService.get('urls.RYOSHI_RPC')),
    };
    ...
    this.account = {
      ...
      RYOSHI: this.web3.BNB.eth.accounts.privateKeyToAccount(
        `0x${this.configService.get('credential.ACCOUNT_PRIVATE_KEY')}`,
      ),
    };
    ...
    this.inkubateContract = {
      ...
      BNB: new this.web3.BNB.eth.Contract(INKUBATE_ABI, INKUBATE_ADDRESS_BSC),
      RYOSHI: new this.web3.RYOSHI.eth.Contract(
        INKUBATE_ABI,
        INKUBATE_ADDRESS_RYOSHI,
      ),
    };
    ...
    this.launchpadContract = {
      ...
      BNB: new this.web3.BNB.eth.Contract(LAUNCHPAD_ABI, LAUNCHPAD_ADDRESS_BSC),
      RYOSHI: new this.web3.RYOSHI.eth.Contract(
        LAUNCHPAD_ABI,
        LAUNCHPAD_ADDRESS_RYOSHI,
      ),
    ...
    this.launchpadContract.BNB.setProvider(this.web3.BNB.currentProvider);
    this.launchpadContract.RYOSHI.setProvider(this.web3.RYOSHI.currentProvider);
  }
  ...
  async deployCollection(
    ...
    try {
      const txObj = {
        ...
        to: this.launchpadContract[network].options.address,
        data: txData,
      };
      console.log(data);
    ...
    try {
      ...
      tokenDatas = await Promise.all(
        tokenIds.map(async (tokenId) => {
          ...
          const url = `${resolveIpfs(tokenUri)}${tokenId}`;
          const decodedUrl = decodeURIComponent(url);

          // Removing null characters from the decoded URL
          const cleanedUrl = decodedUrl.replace(/[^a-zA-Z0-9:/.]/g, '');
          try {
            // Decoding the URL to handle percent-encoded characters

            const res = await axios.get(resolveIpfs(cleanedUrl));

            metadata = {
              name: res?.data?.name ?? tokenId,
              image: resolveIpfs(res.data.image),
